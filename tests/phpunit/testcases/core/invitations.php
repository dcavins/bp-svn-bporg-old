...
class BP_Tests_Invitations extends BP_UnitTestCase {
	...
	public function test_bp_invitations_add_request_request_plus_invite_should_accept() {
		global $wpdb;
		$old_current_user = get_current_user_id();
		...
		$invites = bp_invitations_get_invitations( $u2, $get_invites );
		$this->assertEqualSets( array( $r1 ), wp_list_pluck( $invites, 'id' ) );
